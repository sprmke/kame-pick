...
export function CloudSettingsPanel() {
  ...
  async function importLocal() {
    ...
    try {
      const result = await api.importLocal();
      setImportResult(
        `Imported ${result.imported} candidates (${result.skipped} skipped).` +
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mt-1 text-sm text-zinc-500">
          One-time migration from <code className="text-xs">data/candidates/</code> and SQLite notes into your cloud workspace.
        </p>
        <button
          type="button"
          disabled={importing}
          onClick={importLocal}
          className="mt-4 rounded-lg border border-zinc-300 px-4 py-2 text-sm font-medium hover:bg-zinc-50 disabled:opacity-60 dark:border-zinc-700 dark:hover:bg-zinc-800"
        >
          {importing ? "Importing…" : "Import from local filesystem"}
        </button>
        {importResult && <p className="mt-3 text-sm text-zinc-600 dark:text-zinc-400">{importResult}</p>}
      </div>
    </section>
  );
}
