...
function githubRepoLabel(repoCount: number | null | undefined, errorLabel: string | null): string {
  if (repoCount != null) return `${repoCount} ${repoCount === 1 ? 'repo' : 'repos'}`
  return errorLabel ?? '…'
}
...
export function GithubReposCell({
  ...
  if (!githubUrls?.length) return <span className="text-zinc-500">—</span>
