...
import { api, type GitHubInsights, type GitHubRepoSummary } from "#/lib/api";
...
function GitHubErrorBanner({ error }: { error: string }) {
  const message =
    error === "Rate limited"
      ? "GitHub rate limit — add GITHUB_TOKEN to .env or retry later."
      : error;

  return <p className="mt-4 text-sm text-amber-600">{message}</p>;
}

function GitHubRepoListItem({ repo }: { repo: GitHubRepoSummary }) {
  return (
...
function GitHubStatsGrid({ gh }: { gh: GitHubInsights }) {
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      <StatBox label="Public repos" value={gh.public_repos ?? "—"} />
      <StatBox label="Active repos" value={gh.active_repo_count ?? 0} sub="pushed in last 90 days" />
      <StatBox label="Inactive repos" value={gh.inactive_repo_count ?? 0} sub="no recent pushes" />
      <StatBox label="Total stars" value={gh.total_stars ?? 0} />
    </div>
  );
}

function GitHubProfileMeta({ gh }: { gh: GitHubInsights }) {
  return (
...
function GitHubRecentRepos({ repos }: { repos: GitHubRepoSummary[] }) {
  return (
    <div>
      ...
      <ul className="space-y-2">
        {repos.map((repo) => (
          <GitHubRepoListItem key={repo.url} repo={repo} />
        ))}
      </ul>
    </div>
  );
...
function GitHubTopRepos({ repos }: { repos: GitHubRepoSummary[] }) {
  return (
    <div>
      <p className="mb-2 text-xs font-medium uppercase tracking-wide text-zinc-500">
        Top repos by stars
      ...
      <ul className="flex flex-wrap gap-2">
        {repos.slice(0, 5).map((repo) => (
          <li key={repo.url}>
            ...
            >
              {repo.name}
              {(repo.stars ?? 0) > 0 && <span className="text-zinc-500">★{repo.stars}</span>}
            </a>
          </li>
        ))}
      </ul>
...
function GitHubInsightsBody({ gh }: { gh: GitHubInsights }) {
  return (
    <div className="mt-4 space-y-4">
      ...
      <GitHubStatsGrid gh={gh} />
      <GitHubProfileMeta gh={gh} />

      {(gh.recent_active_repos?.length ?? 0) > 0 && (
        <GitHubRecentRepos repos={gh.recent_active_repos ?? []} />
      )}

      {(gh.top_repos?.length ?? 0) > 0 && <GitHubTopRepos repos={gh.top_repos ?? []} />}
    </div>
  );
}

function GitHubSection({ slug, githubUrls }: { slug: string; githubUrls: string[] }) {
  ...
  return (
    <section className="mt-6">
      ...
      {loading && !gh && <p className="mt-4 text-sm text-zinc-500">Loading GitHub activity…</p>}
      {error && <p className="mt-4 text-sm text-red-600">{error}</p>}
      {gh?.error && !gh.public_repos && <GitHubErrorBanner error={gh.error} />}
      {gh && !gh.error && <GitHubInsightsBody gh={gh} />}
...
function LinkedInSection({ linkedinUrls }: { linkedinUrls: string[] }) {
  return (
...
function PortfolioSection({ portfolioUrls }: { portfolioUrls: string[] }) {
  return (
    <section className="mt-6 border-t border-zinc-100 pt-6 dark:border-zinc-800">
...
export function CandidateLinksPanel({
  ...
  return (
    <Card className="lg:col-span-2">
      ...
      {githubUrls.length > 0 && <GitHubSection slug={slug} githubUrls={githubUrls} />}
      {linkedinUrls.length > 0 && <LinkedInSection linkedinUrls={linkedinUrls} />}
      {portfolioUrls.length > 0 && <PortfolioSection portfolioUrls={portfolioUrls} />}
    </Card>
  );
}
